...
const templateEditSchema = z.object({
  codigo: codeSchema.refine((val) => val.length <= 4, { message: "Código deve ter no máximo 4 caracteres" }),
  nome: z.string().trim().min(3, { message: "Nome deve ter pelo menos 3 caracteres" }).max(100, { message: "Nome deve ter no máximo 100 caracteres" }),
  descricao: z.string().trim().max(200, { message: "Descrição deve ter no máximo 200 caracteres" }).optional().or(z.literal("")),
...
Por ser verdade, passamos a presente declaração.`
  ...
  const handleContentChange = (value: string) => {
    // Sanitize HTML but allow template placeholders
    const sanitized = sanitizeHTML(value);
    setConteudoTemplate(sanitized);
  };

  const onSubmit = (data: TemplateEditFormData) => {
    toast({
      title: "Template actualizado",
      description: `Template "${data.nome}" actualizado com sucesso.`,
    });
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          ...
                          <FormControl>
                            <Input {...field} onChange={(e) => field.onChange(sanitizeString(e.target.value))} />
                          </FormControl>
                          <FormMessage />
                        </FormItem>
                      )}
                    ...
                    render={({ field }) => (
                      <FormItem className="flex flex-row items-start space-x-3 space-y-0">
                        ...
                        <div className="space-y-1 leading-none">
                          <FormLabel>Template activo (disponível para emissão)</FormLabel>
                        </div>
                      </FormItem>
                    )}
                  />
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem className="flex flex-row items-start space-x-3 space-y-0">
                        ...
                        <div className="space-y-1 leading-none">
                          <FormLabel>Requer aprovação da Direcção antes da emissão</FormLabel>
                        </div>
                      </FormItem>
                    )}
                  />
                ...
                <div className="border rounded-lg p-8 bg-white dark:bg-gray-950 min-h-[400px]">
                  ...
                  <div className="text-center mb-8">
                    ...
                    <p className="text-sm text-muted-foreground mt-1">
                      Nº {form.watch("codigo")}-2024-{String(template.numeracaoActual + 1).padStart(4, "0")}
                    </p>
                  </div>
